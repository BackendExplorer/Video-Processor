...
**Python**と**TCPソケット通信**を用いた、メディアファイルの加工・送受信システムです。
クライアントから指示を送るだけで、サーバーがffmpegを使って自動加工・即返却します。
ファイル分割送受信やエラー処理も備え、実用性を意識した設計が特徴です。
...
- **動画圧縮**  
  指定したビットレートで動画サイズを小さくします。

- **解像度変更**  
  フルHD、HD、SDなど希望する解像度に変換します。

- **アスペクト比変更**  
  16:9、4:3、1:1など異なる画面比率に調整します。

- **音声抽出**  
  動画ファイルから音声のみを切り出して保存します。

- **GIF作成**  
  動画の指定区間を切り取り、GIFアニメーションを生成します。

### 作成動機

バックエンド領域に必要なファイル処理やネットワーク通信を実践的に学ぶため、本プロジェクトを開発しました。
単純なWebアプリでは得られない、メディアファイル加工やTCP通信の設計・実装に挑戦しています。
