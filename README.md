...
このサービスでは、TCP通信と独自プロトコル（MMP: Media Manipulation Protocol）を用いて、動画ファイルに対して以下の処理をリアルタイムで実行できます。

- 動画圧縮（ビットレート変更）
- 解像度変更（例：フルHD → HD）
- アスペクト比の変更（例：16:9 → 4:3）
- 音声ファイルへの変換（動画から音声抽出）
- 動画の一部を切り出してGIF作成

クライアントからファイルと加工指示を送信すると、サーバが即時加工を行い、加工済みファイルを返信します。
