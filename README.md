...
flowchart TD
    ...
    ダウンロード実行["加工ファイルダウンロード"]
    ダウンロード失敗か{ダウンロード成功？}
    保存完了["保存完了"]
    
    エラー処理["エラー発生、再試行または終了"]

    Start --> ファイル選択 --> オプション選択 --> アップロード確認 --> アップロード実行 --> アップロード失敗か
    アップロード失敗か -- Yes --> 加工待ち --> 加工完了通知 --> ダウンロード確認 --> ダウンロード実行 --> ダウンロード失敗か
    アップロード失敗か -- No --> エラー処理
    ダウンロード失敗か -- Yes --> 保存完了 --> 終了([終了])
    ダウンロード失敗か -- No --> エラー処理
```
