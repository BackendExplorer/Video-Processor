...
- **課題点**
  ...
  クライアントとの通信、暗号処理、動画変換といった処理が混在しており、
...
- **課題点**
  ...
  複数種類のメディアファイルを扱う上で、受信側がデータの種類や境界を判断しづらく、処理が煩雑になる問題がありました。

<br>

- **解決アプローチ**

  各セクションのサイズ（Json、MediaType、Payload）を8バイトのヘッダーで明示し、
  
  一貫した構造で送信する独自プロトコル（MMP）を設計しました。

<br>

- **得られた成果**

  データ構造が明確になり、受信処理がシンプルに。
  
  メディア種別に応じた柔軟な加工や拡張が容易になりました。
