...
### ゼロから作る！暗号通信 × FFmpeg動画変換システム

### プロトコル・暗号・メディア処理をすべて自前実装したPythonプロジェクト
...
<table>
  ...
  <tr>
    <td rowspan="2">通信・暗号</td>
    <td>TCPソケット</td>
  </tr>
  <tr>
    <td>ハイブリッド暗号方式 (RSA + AES-CFB)</td>
  </tr>
  <tr>
    <td rowspan="2">メディア処理</td>
    <td>FFmpeg (動画圧縮・解像度変更・音声抽出・GIF作成)</td>
  </tr>
  <tr>
    <td>PythonからのFFmpeg操作</td>
  </tr>
  <tr>
    <td>UIフレームワーク</td>
    <td>Streamlit</td>
  ...
  <tr>
    <td>データベース</td>
    <td>SQLite (logs.db)</td>
  </tr>
  <tr>
    <td rowspan="4">インフラ・コンテナ</td>
    <td>Docker</td>
  </tr>
  <tr>
  ...
  <tr>
    <td>CI/CD</td>
    <td>GitHub Actions (ビルド・起動テスト・Docker Hub自動プッシュ)</td>
  </tr>
  <tr>
    <td>バージョン管理</td>
    <td>Git / GitHub</td>
...
- **解決アプローチ**
  ...
  また、動画変換のようなCPUバウンドな処理にはマルチプロセスが適しているため、

  サーバーコンテナ内部でこれを導入し、複数のリクエストを同時に処理できるようにします。
