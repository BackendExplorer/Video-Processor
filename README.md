...
- 大規模化に向けては、まず1台の処理性能を最大化（**垂直スケーリング**）し、

  次に複数台で処理を分散（**水平スケーリング**）、

  さらに**ロードバランサー**でアクセスを最適に振り分けることで、効率的にボトルネックを解消します。
...
1. **垂直スケーリング**
    ...
    より多くのデータを保存するためにストレージを増やしたりします

<br>

2. **水平スケーリング**

    垂直スケーリングにおける物理的なハードウェアの限界を突破するために、サーバの数を増やします。
